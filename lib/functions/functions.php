...
function checkLogin($levels) {
	...
	if ($access == FALSE) {
		header("Location: http://hcs.mkgstudio.pl/ordersystem/login.php");
	}
}
...
function addUser($user, $pass, $email, $manager, $site_url) {
	$user = secureInput($user);
	$pass = secureInput($pass);
	$email = secureInput($email);
...
function contactUs($name, $email, $message, $site_email) {
	...
	//build email to be sent
	$to = 'grzegorz.adamczyk@hcseurope.pl';
	$subject = "New message from " . $email;

	$message = "
			<html>
			  <head>
				<title>Nowa wiadomość od " . $name . " z systemu HCS ORDER</title>
			  </head>
			  <body>
				<h3>WIADOMOŚĆ Z SYSTEMU HCS ORDER ZAPYTANIE</h3>
